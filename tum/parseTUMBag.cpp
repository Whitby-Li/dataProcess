...
int main(int argc, char **argv) {
    ...
    for (rosbag::MessageInstance const m: view) {
        ...
        } else if (topic == "/vrpn_client/raw_transform") {
            geometry_msgs::TransformStampedConstPtr transform_msg = m.instantiate<geometry_msgs::TransformStamped>();
            const auto &rotation = transform_msg->transform.rotation;
            const auto &translation = transform_msg->transform.translation;

            truthOut << transform_msg->header.stamp.toSec() << " " << translation.x << " " << translation.y << " "
        ...
        if (processed_messages % 1000 == 0)
            cout << processed_messages << " / " << num_messages << endl;
    }
