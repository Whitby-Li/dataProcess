...
int main(int argc, char **argv) {
    ...
    for (rosbag::MessageInstance const m: view) {
        ...
        } else if (topic == "/leica/pose/relative") {
            geometry_msgs::PoseStampedConstPtr pose_msg = m.instantiate<geometry_msgs::PoseStamped>();
            const auto &rotation = pose_msg->pose.orientation;
            const auto &translation = pose_msg->pose.position;

            truthOut << pose_msg->header.stamp.toSec() << " " << translation.x << " " << translation.y << " "
